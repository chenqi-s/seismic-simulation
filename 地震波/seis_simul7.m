...
function [P,rho,V,K,input,x_xl,z_xl,t_xl,uz3,uz2,uz1,uy3,uy2,uy1,ux3,ux2,ux1]=seis_simul7(m,n,T)
...
GS=zeros(length(widthnum+1:5:M-widthnum),T/10);
for k=widthnum+1:5:M-widthnum
% 点震源信号参数
...
for i=2:T
    %点（面）震源赋值
     %P2(widthnum+1:end-widthnum,z0)=P2(widthnum+1:end-widthnum,z0)-K(widthnum+1:end-widthnum,z0)*dt*dt*input(i);
     % P2(x0,z0+1)=P2(x0,z0+1)+K(x0,z0)*dt*dt*input(i);
   % P2([x0-25:x0+25],z0)=P2([x0-25:x0+25],z0)-K([x0-25:x0+25],z0)*dt*dt*input(i);
    P2(k,z0)=P2(k,z0)+K(k,z0)*dt*dt*input(i);
...
GS((k-widthnum)/5+0.8,:)=reshape(P(k,5,:),[1,T/10]);
end
