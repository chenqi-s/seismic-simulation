...
%% the main function
function [P,rho,V,K,input,x_xl,z_xl,t_xl]=seis_simul3_1(m,n,T)
...
rho=rho_distrb(M,N);
%rho=readstruct('model/4.png',M,N,2500,4500,-1);
V=v_distrb(M,N);
K=V.^2.*rho;
%点震源信号参数
f0=30;A=5;
x0=M/2;z0=floor(5);t0id=10;
%基础参数：压力张量，时空序列，震源信号
P=zeros(M,N,T);
...
for i=2:T-1
    P(x0,z0,i)=P(x0,z0,i)-K(x0,z0)*dt*dt*input(i);
...
%         +[zeros(M,1),diff(rho,1,2)].*[zeros(M,1),diff(P(:,:,i),1,2)]/dz/dz...
    ...
    if any(any(abs(P(:,:,i+1))>1e10))
        disp('wrong:the matrix may will have NAN!')    %防止溢出
        disp(i)
        return;
    end
end
