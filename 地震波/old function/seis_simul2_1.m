...
function [P,rho,x_xl,z_xl,t_xl]=seis_simul2_1(m,n,T)
...
%
for i=2:T-1
    prx=[zeros(1,N);diff(P(:,:,i),1)]./dx./rho;
    prz=[zeros(M,1),diff(P(:,:,i),1,2)]./dz./rho;
    P(:,:,i+1)=2*P(:,:,i)-P(:,:,i-1)+K.*dt*dt.*(...
        ([zeros(1,N);diff(prx,1)])/dx+[zeros(M,1),diff(prz,1,2)]./dz);
